...
\end{proof}
This appeared first in Terras (1976)\cite{terras76}. See Lemma \ref{lemma:linear_decomposition'}
below for a variant of this formula.
...
\begin{lemma}\label{lemma:decomposition_correction_eq_sum}\lean{CollatzMapBasics.decomposition_correction_eq_sum}\uses{lemma:num_odd_steps_succ}\leanok
...
\begin{lemma}\label{lemma:linear_decomposition'}\lean{CollatzMapBasics.linear_decomposition'}\uses{lemma:linear_decomposition}\leanok
Another form of the linear identity is given by:
\begin{equation}
    T^j(n) = C(j, n) \cdot n + E(j, n).
\end{equation}
\end{lemma}
\begin{proof}\leanok
By substituting the definitions $C(j, n) = 3^{q(j, n)}/2^j$ and $E(j, n) = Q(j, n)/2^j$ into the
linear identity $2^j T^j(n) = 3^{q(j, n)} n + Q(j, n)$.
\end{proof}
