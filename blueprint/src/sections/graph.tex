...
\end{theorem}
\begin{proof}\leanok
($\Rightarrow$) Assume connectivity. For any $n \ge 1$, $n$ is reachable from 1. By Lemma~\ref{lemma:confluence_of_reachable}, there exist $i, j$ such that $T^i(n) = T^j(1)$. Since 1 is in a cycle $\{1, 2\}$, $T^j(1) \in \{1, 2\}$, so $T^i(n)$ eventually hits 1. By Lemma~\ref{lemma:T_iter_to_collatz_iter}, $n$ eventually hits 1 under $C$.
