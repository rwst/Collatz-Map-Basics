...
\begin{definition}\label{def:ParityVector_q}\lean{CollatzMapBasics.ParityVector.q}\leanok
For a parity vector $v$, $q(v)$ is the number of ones (true entries) in $v$.
\end{definition}

\begin{definition}\label{def:ParityVector_size}\lean{CollatzMapBasics.ParityVector.size}\leanok
The size of a parity vector $v$, denoted $|v|$, is its length.
\end{definition}

\begin{definition}\label{def:ParityVector_onesRatio}\lean{CollatzMapBasics.ParityVector.onesRatio}\leanok
The ones-ratio of a parity vector $v$ is the rational number $q(v) / |v|$. If the vector is empty, the ratio is defined to be 0.
\end{definition}

\begin{definition}\label{def:ParityVector_V}\lean{CollatzMapBasics.ParityVector.V}\leanok
For $j, n \in \mathbb{N}$, the parity vector $V(j, n)$ of length $j$ for the Collatz sequence starting at $n$ is defined as
\begin{equation}
    V(j, n) = (X(n), X(T(n)), \dots, X(T^{j-1}(n))).
\end{equation}
\end{definition}

\begin{lemma}\label{lemma:V_length}\lean{CollatzMapBasics.ParityVector.V_length}\leanok
...
\begin{lemma}\label{lemma:V_get}\lean{CollatzMapBasics.ParityVector.V_get}\leanok
...
\begin{definition}\label{def:ParityVector_Precedes}\lean{CollatzMapBasics.ParityVector.Precedes}\leanok
